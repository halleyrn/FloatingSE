...
from numpy import array, dot, minimum, asarray, round, isnan, cos, sinh, sin, cosh, atleast_1d
...
class Spar(Component):
    ...
    def execute(self):
        ...
        kg_top = (rna_mass * rna_keel_to_cg + tower_mass * tower_keel_to_cg)
        ...
        if isnan(sum_forces_x).any() or sum_forces_x[-1] > (-total_force/1000.):
            ...
            for j in range(1, len(sum_forces_x.tolist())):
                if sum_forces_x[j]< (-total_force/1000.): 
                    ...
                    Y2 = X_Offset[j]
                    Y1 = X_Offset[j-1]
            max_offset = (Y1+(-total_force/1000.-X1)*(Y2-Y1)/(X2-X1))*offset_amplification_factor
            i=0
            while sum_forces_x[i]> (total_force/1000.):
                i+=1
            min_offset = (X_Offset[i-1]+(total_force/1000.-sum_forces_x[i-1])*(X_Offset[i]-X_Offset[i-1])/(sum_forces_x[i]-sum_forces_x[i-1]))*offset_amplification_factor
        # unity checks! 
            ...
            elif self.load_condition == 'number_of_rings': 
                self.max_offset_unity = max_offset/self.intact_mooring[1]
                self.min_offset_unity = min_offset/self.intact_mooring[0]
        M_total = rna_wind_moment+tower_wind_moment+sum(section_wind_moments)+sum(section_currentwave_moments)+(-total_force*(mooring_keel_to_cg-kg_tension))+(rna_mass*gravity*-rna_center_of_gravity_x)
        self.heel_angle = (M_total/pitch_k)*180./pi
        ##### API BULLETIN #####    
        # shell data
        RO = outer_diameters/2.  # outer radius 
        R = RO-wall_thicknesses/2. # radius to centerline of wall/mid fiber radius 
        # ring data 
        LR = section_lengths/(number_of_rings+1.) # number of ring spacing
        #shell and ring data
        RF = RO - stiffener_web_height  # radius to flange
        MX = LR/(R*wall_thicknesses)**0.5  # geometry parameter
        # effective width of shell plate in longitudinal direction 
        LE=array([0.]*number_of_sections)
        for i in range(0,number_of_sections):
            if MX[i] <= 1.56: 
                LE[i]=LR[i]
            else: 
                LE = 1.1*(2*R*wall_thicknesses)**0.5+stiffener_web_thickness 
        # ring properties with effective shell plate
        AER = stiffener_area+LE*wall_thicknesses  # cross sectional area with effective shell 
        YENA = (LE*wall_thicknesses*wall_thicknesses/2 + stiffener_web_height*stiffener_web_thickness*(stiffener_web_height/2+wall_thicknesses) + stiffener_flange_thickness*stiffener_flange_width*(stiffener_flange_thickness/2+stiffener_web_height+wall_thicknesses))/AER 
        IER = stiffener_moment_of_inertia+stiffener_area*(stiffener_yna+wall_thicknesses/2.)**2*LE*wall_thicknesses/AER+LE*wall_thicknesses**3/12. # moment of inertia
        RC = RO-YENA-wall_thicknesses/2. # radius to centroid of ring stiffener 
        # set loads (0 mass loads for external pressure) 
        MBALLAST = permanent_ballast_mass + fixed_ballast_mass + water_ballast_mass # sum of all ballast masses
        W = (rna_mass + tower_mass + MBALLAST + mass) * gravity
        P = water_density * gravity* abs(end_elevations)  # hydrostatic pressure at depth of section bottom 
        ...
        BETAc = (e*wall_thicknesses/(4*RO**2*Dc))**0.25 # parameter beta
        TWS = stiffener_area/stiffener_web_height
        dum1 = BETAc*LR
        KT = 8*BETAc**3 * Dc * (cosh(dum1) - cos(dum1))/ (sinh(dum1) + sin(dum1))
        KD = e * TWS * (RO**2 - RF**2)/(RO * ((1+poisson_ratio) * RO**2 + (1-poisson_ratio) * RF**2))
        dum = dum1/2. 
        ...
        global ZM
        ZM = 12*(MX**2 * (1-poisson_ratio**2)**.5)**2/pi**4
        for i in range(0,number_of_sections):
        ...
        FREL = CTHETAL * pi**2 * e * (wall_thicknesses/LR)**2 / (12*(1-poisson_ratio**2)) # elastic
        ...
        # axial compression and bending 
        AC = stiffener_area/(LR*wall_thicknesses) # Ar bar 
        ALPHAX = 0.85/(1+0.0025*(outer_diameters/wall_thicknesses))
        ...
        FREG = ALPHATHETAG*PEG*RO*KTHETAG/wall_thicknesses # elastic 
        ...
        # factor of safety
        FOS = 1.25
        if load_condition == 'number_of_rings' or load_condition == 'n': 
            FOS = 1.65
        ...
        self.ballast_mass = permanent_ballast_mass + fixed_ballast_mass + water_ballast_mass
        self.system_total_mass = shell_bulkhead_ring_mass + permanent_ballast_mass + fixed_ballast_mass + water_ballast_mass + self.mooring_mass
