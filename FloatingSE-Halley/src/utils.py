...
def curWaveDrag(Hs, Tp, WD, ODT, ODB, ELS, SL, CG, VDOT, G, WDEN): 
    ...
    for i in range(1, 11):
        ...
        for j in range(JS, JE+1):
            ...
            A = 0. 
            if Hs != 0.: 
                V += waveU(H, Tp, k, D2, WD, dtheta*(j-1))
            ...
            if V != 0.: 
                ...
                f = 0.
            if Hs != 0:
                f += inertialForce(D, 1, DL, A, VDOT, WDEN)
            if f > fmax:
                fmax = f
                JMAX[i-1] = j
        m += fmax*L2
...
def windDrag(TLEN, TBOD, TTOD, WREFS, WREFH, ALPHA, FB, ADEN, GF, UCG):
...
def thrust_table(size_of_turbine, ADEN, RWA): 
    ...
    elif size_of_turbine == '5MW':
        Ct = array([0.001, 0.001, 0.001, 0.277, 0.903, 0.720, 0.614, 0.545, 0.500, 0.479, 0.463, 0.437, 0.321, 0.241, 0.191, 0.155, 0.128, 0.108, 0.093, 0.080, 0.070, 0.062, 0.055, 0.049, 0.044, 0.040])
    ...
    thrust = array(0.5*ADEN*wind**2.*RWA*Ct/1000.)
...
def sys_print(example):
    """Prints the results of the optimization in terminal."""
...
def save_sys_log(example):
    day_time = datetime.datetime.now().strftime("%m/%d/%Y %H:%M:%S\n")
    if os.path.exists('Results.txt'):
        results = open("Results.txt",  'a')
        results.write('\n\n\n')
    else:
        results = open("Results.txt",  'w')
    results.write("Run on " + day_time)
    results.write("-----Cost Inputs-----\n")
    results.write("cost of straight columns:  %.2f USD/ton\n" % example.straight_col_cost)
    results.write("cost of tapered columns:  %.2f USD/ton\n" % example.tapered_col_cost)
    results.write("cost of outfitting:  %.2f USD/ton\n" % example.outfitting_cost)
    results.write("cost of ballast:  %.2f USD/ton\n" % example.ballast_cost)
    results.write("-----Cost Outputs-----\n")
    results.write("cost of spar (or hull):  %.2f USD\n" % example.spar.spar_cost)
    results.write("cost of outfitting:  %.2f USD\n" % example.spar.outfit_cost)
    results.write("cost of fixed ballast:  %.2f USD\n" % example.spar.ballasts_cost)
    results.write("cost of mooring, fixed ballast, outfitting, and spar:  %.2f USD\n" % example.spar.total_cost)
    results.write("-----Environmental Inputs-----\n")
    results.write("gravity:  %.3f m/s**2\n" % example.gravity)
    results.write("air density:  %.3f kg/m**3\n" % example.air_density)
    results.write("water depth:  %.3f m\n" % example.water_depth)
    results.write("water density:  %.3f kg/m**3\n" % example.water_density)
    results.write("load condition:  %s\n" % example.load_condition)
    results.write("significant wave height:  %.3f m\n" % example.significant_wave_height)
    results.write("significant wave period:  %.3f m\n" % example.significant_wave_period)
    results.write("wind reference wave height:  %.3f m\n" % example.wind_reference_height)
    results.write("wind reference wave speed:  %.3f m/s\n" % example.wind_reference_speed)
    results.write("alpha:  %.3f\n" % example.alpha)
    results.write("gust:  %.3f\n" % example.gust_factor)
    results.write("-----Material Inputs-----\n") #add units for everything under here
    results.write("material density:  %.3f\n" % example.material_density)
    results.write("E:  %e\n" % example.E)
    results.write("nu:  %.3f\n" % example.nu)
    results.write("yield stress:  %e\n" % example.yield_stress)
    results.write("permanent ballast density:  %.3f\n" % example.permanent_ballast_density)
    results.write("fixed ballast density:  %.3f\n" % example.fixed_ballast_density)
    results.write("-----RNA Inputs-----\n")
    results.write("RNA mass:  %.3f\n" % example.RNA_mass)
    results.write("rotor diameter:  %.3f\n" % example.rotor_diameter)
    results.write("RNA center of gravity y:  %.3f\n" % example.RNA_center_of_gravity_y)
    results.write("RNA center of gravity x:  %.3f\n" % example.RNA_center_of_gravity_x)
    results.write("cut out speed:  %.3f\n" % example.cut_out_speed)
    results.write("turbine size:  %s\n" % example.example_turbine_size)
    results.write("-----RNA Outputs-----\n")
    results.write("RNA keel to CG:  %.3f\n" % example.towerRNA.RNA_keel_to_CG)
    results.write("RNA wind force:  %.3f\n" % example.towerRNA.RNA_wind_force)
    results.write("-----Tower Inputs-----\n")
    results.write("base outer diameter:  %.3f\n" % example.tower_base_outer_diameter)
    results.write("top outer diameter:  %.3f\n" % example.tower_top_outer_diameter)
    results.write("tower length:  %.3f\n" % example.tower_length)
    results.write("tower mass:  %.3f\n" % example.tower_mass)
    results.write("user tower CG:  %.3f\n" % example.user_tower_cg)
    results.write("-----Tower Outputs-----\n")
    results.write("tower center of gravity:  %.3f\n" % example.towerRNA.tower_center_of_gravity)
    results.write("tower keep to CG:  %.3f\n" % example.towerRNA.tower_keel_to_CG)
    results.write("tower wind forces:  %.3f\n" % example.towerRNA.tower_wind_force)
    results.write("-----Stiffener Inputs-----\n")
    results.write("stiffener index:  %d\n" % example.stiffener_index)
    results.write("stiffener curve fit:  %s\n" % str(example.stiffener_curve_fit))
    results.write("neutral axis:  %.3f\n" % example.neutral_axis)
    results.write("-----Stiffener Outputs-----\n")
    results.write("cross-sectional area of member:  %.3f\n" % example.spar.Ar)
    results.write("overall depth of member, parallel to X-axis:  %.3f\n" % example.spar.d)
    results.write("thickness of web of member:  %.3f\n" % example.spar.tw)
    results.write("width of flange of member:  %.3f\n" % example.spar.bf)
    results.write("thickness of flange of member:  %.3f\n" % example.spar.tf)
    results.write("distance from outer face of flange to web toe to fillet:  %.3f\n" % example.spar.yna)
    results.write("moment of inertia for a stiffener:  %.3f\n" % example.spar.Ir)
    results.write("check for flange compactness:  %.3f\n" % example.spar.flange_compactness)
    results.write("check for web compactness:  %.3f\n" % example.spar.web_compactness)
    results.write("-----Section Inputs-----\n")
    results.write("outer diameter:  [%s]\n" % ', '.join(map(str, example.spar_outer_diameter)))
    results.write("wall thickness:  [%s]\n" % ', '.join(map(str, example.wall_thickness)))
    results.write("elevations:  [%s]\n" % ', '.join(map(str, example.spar_elevations)))
    results.write("bulk head:  [%s]\n" % ', '.join(map(str, example.bulk_head)))
    results.write("number of rings:  [%s]\n" % ', '.join(map(str, example.number_of_rings)))
    results.write("number of sections:  %.3f\n" % example.number_of_sections)
    results.write("-----Section Outputs-----\n")
    results.write("axial load - local buckling unity:  [%s]\n" %
                  ', '.join(map(str, map('{0:f}'.format, example.spar.VAL))))
    results.write("axial load - general instability unity:  [%s]\n" %
                  ', '.join(map(str, map('{0:f}'.format, example.spar.VAG))))
    results.write("external pressure - local buckling unity:  [%s]\n" %
                  ', '.join(map(str, map('{0:f}'.format, example.spar.VEL))))
    results.write("external pressure - general instability unity:  [%s]\n" %
                  ', '.join(map(str, map('{0:f}'.format, example.spar.VEG))))
    results.write("-----Mooring Inputs-----\n")
    results.write("fairlead depth: %.3f\n" % example.fairlead_depth)
    results.write("scope ratio: %.3f\n" % example.scope_ratio)
    results.write("number of mooring lines: %d\n" % example.number_of_mooring_lines)
    results.write("diameter: %.3f\n" % example.mooring_diameter)
    results.write("type: %s\n" % example.mooring_type)
    results.write("user minimum breaking load: %.3f\n" % example.user_MBL)
    results.write("user wet mass per length: %.3f\n" % example.user_WML)
    results.write("user E modulus: %.3f\n" % example.user_AE_storm)
    results.write("user cost per length: %.3f\n" % example.user_MCPL)
    results.write("anchor type: %s\n" % example.anchor_type)
    results.write("user anchor cost: %.3f\n" % example.user_anchor_cost)
    results.write("miscellaneous cost factor in percent: %.3f\n" % example.misc_cost_factor)
    results.write("fairlead offset from shell: %.3f\n" % example.fairlead_offset_from_shell)
    results.write("user mass density in air: %.3f\n" % example.user_mass_density_air)
    results.write("user elemental axial stiffness: %.3f\n" % example.user_EA_stiffness)
    results.write("radius to anchors from platform center line: %.3f\n" % example.anchor_radius)
    results.write("-----Mooring Outputs-----\n")
    results.write("mooring keel to CG: %.3f\n" % example.mapMooring.mooring_keel_to_CG)
    results.write("mooring vertical load in all mooring lines: %.3f\n" % example.mapMooring.mooring_vertical_load)
    results.write("linearized horizontal stiffness: %.3f\n" % example.mapMooring.mooring_horizontal_stiffness)
    results.write("linearized vertical stiffness: %.3f\n" % example.mapMooring.mooring_vertical_stiffness)
    results.write("sum of forces in x direction: [%s]\n" %
                  ', '.join(map(str, map('{0:f}'.format, example.mapMooring.sum_forces_x))))
    results.write("X offsets in discretization: [%s]\n" %
                  ', '.join(map(str, map('{0:f}'.format, example.mapMooring.offset_x))))
    results.write("range of damaged mooring: [%s]\n" %
                  ', '.join(map(str, map('{0:f}'.format, example.mapMooring.damaged_mooring))))
    results.write("range of intact mooring: [%s]\n" %
                  ', '.join(map(str, map('{0:f}'.format, example.mapMooring.intact_mooring))))
    results.write("total mass of mooring: %.3f\n" % example.mapMooring.mooring_mass)
    results.write("tension of the mooring system at initial position: %.3f\n" % example.mapMooring.pretension)
    results.write('used minimum breaking load: %.3f\n' % example.mapMooring.MBL)
    results.write("used length of a mooring line: %.3f\n" % example.mapMooring.scope)
    results.write("used wet mass per length: %.3f\n" % example.mapMooring.WML)
    results.write("used E modulus: %.3f\n" % example.mapMooring.AE_storm)
    results.write("used mooring cost per length: %.3f\n" % example.mapMooring.MCPL)
    results.write("used anchor cost: %.3f\n" % example.mapMooring.anchor_cost)
    results.write("used mass density in air: %.3f\n" % example.mapMooring.mass_density_air)
    results.write("-----Platform Inputs-----\n")
    results.write("shell mass factor: %.3f\n" % example.shell_mass_factor)
    results.write("bulkhead mass factor: %.3f\n" % example.bulkhead_mass_factor)
    results.write("ring mass factor: %.3f\n" % example.ring_mass_factor)
    results.write("outfitting factor: %.3f\n" % example.outfitting_factor)
    results.write("spar mass factor: %.3f\n" % example.spar_mass_factor)
    results.write("permanent ballast height: %.3f\n" % example.permanent_ballast_height)
    results.write("fixed ballast height: %.3f\n" % example.fixed_ballast_height)
    results.write("offset amplification factor: %.3f\n" % example.offset_amplification_factor)
    results.write("-----Platform Outputs-----\n")
    results.write("spar mass: %.3f\n" % example.spar.spar_mass)
    results.write("ballast mass: %.3f\n" % example.spar.ballast_mass)
    results.write("system total mass: %.3f\n" % example.spar.system_total_mass)
    results.write("shell mass: %.3f\n" % example.spar.shell_mass)
    results.write("bulkhead mass: %.3f\n" % example.spar.bulkhead_mass)
    results.write("stiffener mass: %.3f\n" % example.spar.stiffener_mass)
    results.write("outfitting mass: %.3f\n" % example.spar.outfitting_mass)
    results.write("permanent ballast mass: %.3f\n" % example.spar.permanent_ballast_mass)
    results.write("fixed ballast mass: %.3f\n" % example.spar.fixed_ballast_mass)
    results.write("-----Sizing Outputs-----\n")
    results.write("platform stability check: %.3f\n" % example.spar.platform_stability_check)
    results.write("heel angle: %.3f\n" % example.spar.heel_angle)
    results.write("min offset unity: %.3f\n" % example.spar.min_offset_unity)
    results.write("max offset unity: %.3f\n" % example.spar.max_offset_unity)
    results.write("water ballast height: %.3f\n" % example.spar.water_ballast_height)
    results.write("water ballast mass: %.3f\n" % example.spar.water_ballast_mass)
    results.write("total force: %.3f\n" % example.spar.total_force)
    results.write("surge period: %.3f\n" % example.spar.surge_period)
    results.write("heave period: %.3f\n" % example.spar.heave_period)
    results.write("pitch stiffness: %.3f\n" % example.spar.pitch_stiffness)
    results.write("pitch period: %.3f\n" % example.spar.pitch_period)
    results.close()
