...
def save_sys_log(example):
    ...
    results.write("cost of straight columns:  %f USD/ton\n" % example.straight_col_cost)
    results.write("cost of tapered columns:  %f USD/ton\n" % example.tapered_col_cost)
    results.write("cost of outfitting:  %f USD/ton\n" % example.outfitting_cost)
    results.write("cost of ballast:  %f USD/ton\n" % example.ballast_cost)
    results.write("-----Cost Outputs-----\n")
    results.write("cost of spar (or hull):  %f USD\n" % example.spar.spar_cost)
    results.write("cost of outfitting:  %f USD\n" % example.spar.outfit_cost)
    results.write("cost of fixed ballast:  %f USD\n" % example.spar.ballasts_cost)
    results.write("cost of mooring, fixed ballast, outfitting, and spar:  %f USD\n" % example.spar.total_cost)
    ...
    results.write("unity check for axial load - local buckling:  %.3f\n" % example.spar.VAL)
    results.write("unity check for axial load - general instability:  %.3f\n" % example.spar.VAG)
    results.write("unity check for external pressure - local buckling:  %.3f\n" % example.spar.VEL)
    results.write("unity check for external pressure - general instability:  %.3f\n" % example.spar.VEG)
    ...
    results.write("sum of forces in x direction: %.3f\n" % example.mapMooring.sum_forces_x)
    results.write("X offsets in discretization: %.3f\n" % example.mapMooring.offset_x)
    results.write("range of damaged mooring: %.3f\n" % example.mapMooring.damaged_mooring)
    results.write("range of intact mooring: %.3f\n" % example.mapMooring.intact_mooring)
    ...
    results.write('calculated minimum breaking load: %.3f\n' % example.mapMooring.MBL)
    results.write("calculated length of a mooring line: %.3f\n" % example.mapMooring.scope)
    results.write("calculated wet mass per length: %.3f\n" % example.mapMooring.WML)
    results.write("calculated E modulus: %.3f\n" % example.mapMooring.AE_storm)
    results.write("calculated mooring cost per length: %.3f\n" % example.mapMooring.MCPL)
    results.write("calculated anchor cost: %.3f\n" % example.mapMooring.anchor_cost)
    results.write("calculated mass density in air: %.3f\n" % example.mapMooring.mass_density_air)
